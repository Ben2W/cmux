...
impl Surface {
    ...
    /// Run `f` with exclusive terminal access and its matching content
    /// generation. The generation is read while the terminal lock is held so
    /// semantic ranges cannot be tagged with a stale snapshot.
