    ...
    func clearSupersededAgentHookSessions(
        ...
        for record in records {
            let resumeClearOutcome = clearAgentSurfaceResumeBindingOutcome(
                ...
                agentEventTime: owner.runtimeStatusEventTime
            )
            ...
            do {
                _ = try sendV1Command(
                    "clear_agent_pid \(pidKey) --tab=\(record.workspaceId)\(socketPanelOption(record.surfaceId)) --clear-status --require-owned-key\(agentEventTimeOption(owner.runtimeStatusEventTime))",
                    client: client
                )
                clearedRecords.append(record)
            } catch {
