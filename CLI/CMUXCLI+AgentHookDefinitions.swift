...
extension CMUXCLI {
    ...
    static func agentHookCaptureTimeShell() -> String {
        ...
        [
            ...
            #"lock="$clock_dir/lock"; state="$clock_dir/state"; ( exec 9>>"$lock" || exit 1; if ! /usr/bin/lockf -s -t 0 9; then exit 1; fi;"#,
            ...
            #"if [ -n "$formatted_at" ]; then epoch="${formatted_at%.*}"; fraction="${formatted_at#*.}"; if [ -n "$epoch" ] && [ -n "$fraction" ]; then case "$epoch" in *[!0-9]*) ;; *) case "$fraction" in *[!0-9]*) ;; *) current_micros=$((10#$epoch * 1000000 + 10#$fraction));; esac;; esac; fi; fi;"#,
