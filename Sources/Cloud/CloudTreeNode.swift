...
final class CloudTreeNode: NSObject {
    ...
    var machine: SurfaceMachineID {
        ...
        case .display(let resource, _, _), .port(let resource): return resource.machine
    ...
    var dragResource: SurfaceResource? {
        ...
        case .display(let resource, _, _), .port(let resource): return resource
    ...
    private static func cloudChildren(machine: SurfaceMachineID, info: SurfaceMachineInfo?, snapshot: SurfaceCatalogSnapshot) -> [CloudTreeNode] {
        ...
        case .connected, .notApplicable:
            ...
            children.append(CloudTreeNode(
                ...
                children: terminals.map {
                    // A pool row carries a view only when the target is unique.
                    // Multi-view terminals must be selected under a workspace.
                    terminalNode(
                        $0,
                        snapshot: snapshot,
